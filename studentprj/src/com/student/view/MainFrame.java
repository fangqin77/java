...
public class MainFrame extends JFrame {
    ...
    public void initMenus() {
        ...
        changeClassMenuItem.addActionListener(e -> {
            ...
            initMenus();
            List<String> classList = loadClasses();
            ChangeClassPanel changeClassPanel = new ChangeClassPanel(this, classList);
        ...
        addStudentMenuItem.addActionListener(e -> {
            if (Constant.CLASS_PATH.isEmpty()) {
                ...
                initMenus();
                List<String> classList = loadClasses();
                this.getContentPane().add(new StudentAddPanel(classList), BorderLayout.CENTER);
                this.getContentPane().repaint();
                this.getContentPane().validate();
            }
        });
    ...
    private List<String> loadClasses() {
        ...
        if (files != null) {
            for (File file : files) {
                if (file.isFile()) {
                    classNames.add(file.getName().replace(".txt", "")); // 去掉文件扩展名
                }
            }
        }
        return classNames;
